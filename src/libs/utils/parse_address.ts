...
export function parseAddress(address:any):address{
    ...
    let data:address = {
        ...
        free_form_address: (findLongNames(address, 'street_number') + " " + findLongNames(address, 'route') +" " +  findLongNames(address, 'locality') +" " +  findLongNames(address, 'administrative_area_level_1') +" " +  findLongNames(address, 'postal_code')).trim(),
        local_name: "",
