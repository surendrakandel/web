...
export const zodPickupLocation = z.object({
	...
	business_hours: z.object({}).optional().nullable(),
...
export type zodPickupLocationType = z.infer<typeof zodPickupLocation>;

export function newPickupLocation(): zodPickupLocationType{
    let data: zodPickupLocationType = {
		id: '',
		type: '',
		company_name: '',
		address: newAddress(addressType.pickup),
		contact: newContact(),
		business_hours: { start: '', end: '' },
		PICKUP_LOCATION_NONE: false,
		PICKUP_LOCATION_WITH_DOCK: true,
		INSIDE_PICKUP: false,
		...
		shipper_additional_services_error: makeInfo({
			valid: true,
			id: 'shipper_additional_services_err',
			error_message: 'quote pickup zipcode and current zipcode does not match. please verify pickup zipcode or update the quote.',
			clas: "firstshipper_errors",
			path: ''
		}),
